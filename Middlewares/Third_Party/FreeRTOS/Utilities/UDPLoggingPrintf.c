...
uint32_t ulSeconds, ulMilliSeconds, ulMicroSeconds;
	...
	vTaskSuspendAll();
	{
		ullCurrentTime = ullGetRunTimeCounterValue();
		ulSeconds = ( uint32_t ) ( ullCurrentTime / 10000ull );
		ullCurrentTime = ullCurrentTime % 10000ull;
		ulMilliSeconds = ( uint32_t ) ( ullCurrentTime / 10ull );
		ulMicroSeconds = ( uint32_t ) ( ullCurrentTime % 100ull );
...
//		pxEndPoint = FreeRTOS_FirstEndPoint( NULL );
...
#if( ipconfigMULTI_INTERFACE != 0 )
	xLocalAddress.sin_addr = pxEndPoint->ulDefaultIPAddress;
#else
	...
	#if( ipconfigMULTI_INTERFACE != 0 )
		if( pxNetworkInterface->pfGetPhyLinkStatus( pxNetworkInterface->pvArgument ) != pdFALSE )
	#else
	...
	#endif
		{
			...
			for( lLines = 0; lLines < configUDP_LOGGING_MAX_MESSAGES_IN_BUFFER; lLines++ )
			{
				...
				NetworkEndPoint_t *pxEndPoint;
					...
					{
					NetworkEndPoint_t *pxNextEndPoint = FreeRTOS_NextEndPoint( NULL, pxEndPoint );
